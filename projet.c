...
int minimax(Arbre a, int profondeur, int alpha, int beta, int n, int (*evaluate)(Arbre, int)){
    ...
    if (a->joueur == 2){
        ...
        for (i = 0; i < 4; i++){
            ...
            haschild = 1;
            eval = minimax(fils[i], profondeur - 1, alpha, beta, n, evaluate);
...
int choisir_direction(Arbre a, int n, int dif){
    Arbre fils[4] = {a->N1, a->N2, a->N3, a->N4};
    ...
    if (dif == 0){
        for (i=0; i<4; i++) {
            ...
            score = minimax(fils[i], 5, -100, 100, n, evaluate_easy);
            if (score > bestscore) {
                bestscore = score;
                bestdir = i + 1;
        ...
        for (i=0; i<4; i++) {
            ...
            score = minimax(fils[i], 5, -100, 100, n, evaluate_medium);
            if (score > bestscore) {
                bestscore = score;
                bestdir = i + 1;
                // printf("dir %d -> score %d\n", i+1, score);
        ...
        for (i=0; i<4; i++) {
            ...
            score = minimax(fils[i], 5, -100, 100, n, evaluate_hard);
            if (score > bestscore) {
                bestscore = score;
                bestdir = i + 1;
                // printf("dir %d -> score %d\n", i+1, score);
    ...
    if(start==1) {    
        ...
        while(!WindowShouldClose() &&zresultat==0 &&resultat==0 && taille<win && ztaille<win){
            ...
            if(frame_counter>=40){ //tour de jeu toutes les 40 frames
                ...
                zdir = choisir_direction(a, n, diff);
                supprimer_arbre(a, n);
